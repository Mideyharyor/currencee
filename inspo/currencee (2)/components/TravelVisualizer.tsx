...
import { generateTravelImage, checkApiKey, selectApiKey } from '../services/geminiService';
...
export const TravelVisualizer: React.FC<TravelVisualizerProps> = ({ targetCurrency, currencyName }) => {
    ...
    try {
      ...
      if (!hasKey) {
        ...
        if (!(await checkApiKey())) {
          throw new Error("API Key selection required to use this feature.");
        }
      }
      ...
      const result = await generateTravelImage(finalPrompt, size);
        ...
        {imageUrl && (
          <div className="mt-8 rounded-2xl overflow-hidden shadow-2xl border border-white/10 relative group">
             <div className="absolute top-4 right-4 z-10 opacity-0 group-hover:opacity-100 transition-opacity">
                <a 
                  href={imageUrl} 
                  download={`travel-${targetCurrency}-${Date.now()}.png`}
